...
logger = logging.getLogger("seed_new_synthetic_data")
...
def create_bbox_polygon(lat: float, lon: float, dlat: float = 0.5, dlon: float = 1.0) -> str:
    ...
    return (
        f"SRID=4326;POLYGON(({min_lon} {min_lat}, {max_lon} {min_lat}, {max_lon} {max_lat}, "
        f"{min_lon} {max_lat}, {min_lon} {min_lat}))"
    )

def read_csv_from_dir(base_dir: str, relative_path: str):
    """Read a CSV file from the synthetic data directory and return a list of dicts."""
...
def seed_grid_cells_and_risk_cells(conn, data_dir):
    logger.info("Seeding risk_cells from grid_cells_2026.csv...")
    cells = read_csv_from_dir(data_dir, "grid_cells_2026.csv")
    ...
    for row in cells:
        ...
        risk_cell_records.append((
            r_id, poly_wkt, base_time, ice_risk, iceberg_risk, weather_risk, current_risk, composite, cat, 1.0,
            json.dumps({"has_gap": False}),
            json.dumps({"cell_id": cell_id, "lat_band": row.get("lat_band"), "lon_band": row.get("lon_band")})
        ))
    sql = """
...
def seed_sea_ice(conn, data_dir):
    logger.info("Seeding sea_ice_observations...")
    rows = read_csv_from_dir(data_dir, "sea_ice_synthetic_2026.csv")
    records = []
    for r in rows:
        ...
        records.append((obs_id, dt, poly_wkt, conc, thickness, ice_type, src, 1.0))
    sql = """
...
def seed_weather(conn, data_dir):
    logger.info("Seeding weather_observations...")
    rows = read_csv_from_dir(data_dir, "weather_synthetic_2026.csv")
    records = []
    for r in rows:
        sample_id = r["sample_id"]
        obs_id = get_uuid(sample_id)
        ...
        src = r.get("data_source_type", "SYNTHETIC_PROTOTYPE")
        records.append((obs_id, dt, pt_wkt, w_speed, w_dir, temp, wave_h, press, src))
    sql = """
...
def seed_ocean(conn, data_dir):
    logger.info("Seeding ocean_observations...")
    rows = read_csv_from_dir(data_dir, "ocean_currents_synthetic_2026.csv")
    records = []
    seen_ids = set()
    for r in rows:
        sample_id = r["sample_id"]
        ...
        src = r.get("data_source_type", "SYNTHETIC_PROTOTYPE")
        records.append((obs_id, dt, pt_wkt, c_speed, c_dir, sst, wave_info, src))
    sql = """
...
def seed_icebergs(conn, data_dir):
    logger.info("Seeding icebergs and trajectories...")
    rows = read_csv_from_dir(data_dir, "iceberg_trajectory_synthetic_2026.csv")
    unique_berg_ids = sorted({r["iceberg_id"] for r in rows})
    ...
    for r in rows:
        ...
        detection_records.append((
            d_id, b_uuid, dt, pt_wkt, est_size, extent_wkt, 0.95, "Synthetic SAR Sentinel-1",
            json.dumps({"sample_id": sample_id, "wind_speed": float(r["wind_speed_m_s"]), "sea_ice_conc": float(r["sea_ice_concentration"])})
        ))
        ...
        prediction_records.append((
            p_id, b_uuid, dt, horizon, "hours", pred_pt_wkt, path_json, uncert_wkt, 0.88, "synthetic-xgb-v1"
        ))
    sql_det = """
...
def seed_routes(conn, data_dir):
    logger.info("Seeding routes...")
    routes_raw = read_csv_from_dir(data_dir, "routes_synthetic_2026.csv")
    waypoints_raw = read_csv_from_dir(data_dir, "route_waypoints_synthetic_2026.csv")
    wp_by_route = {}
    for wp in waypoints_raw:
        r_id = wp["route_id"]
        wp_by_route.setdefault(r_id, []).append(wp)
    ...
    for r in routes_raw:
        ...
        route_records.append((
            r_uuid, orig, dest, vessel_uuid, dep_time, line_wkt, dist_nm, arr_time, est_fuel, risk_score, obj_type, "synthetic-a-star-v1"
        ))
    sql = """
...
def main():
    data_dir = sys.argv[1] if len(sys.argv) > 1 else r"C:\\Users\\ASUS\\Desktop\\offshore\\synthetic_data"
    logger.info("Starting synthetic database seeding from directory: %s", data_dir)
    ...
    try:
        ...
        seed_vessels(conn)
        seed_grid_cells_and_risk_cells(conn, data_dir)
        seed_sea_ice(conn, data_dir)
        seed_weather(conn, data_dir)
        seed_ocean(conn, data_dir)
        seed_icebergs(conn, data_dir)
        seed_routes(conn, data_dir)
        seed_alerts(conn, data_dir)
