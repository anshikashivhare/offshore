...
from app.api.v1.endpoints.routes import (_build_risk_grid, astar_planner,
                                         shortest_planner)
            ...
            async with AsyncSessionLocal() as db:
                ...
                try:
                    if request.objective_type.value == "shortest":
                        route_create = await shortest_planner.plan_route(
                            request, vessel, risk_grid
                        )
                    else:
                        route_create = await astar_planner.plan_route(
                            request, vessel, risk_grid
                        )
                    ...
                    logger.error(
                        "Error evaluating route in background task: %s", eval_exc
                    )
            ...
            try:
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    update_task = asyncio.create_task(
                        route_update_loop(websocket, request)
                    )
