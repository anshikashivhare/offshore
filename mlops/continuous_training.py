...
from mlops.drift_detection import load_reference_statistics, detect_drift
from mlops.retrain_trigger import should_retrain
from mlops.model_registry import promote_if_better
from mlops.experiment_log import get_latest_metric, get_experiment_history
...
MODEL_CONFIGS = {
    ...
    "seaice_convlstm": {
        ...
        "drift_reference_path": get_model_path("reference_stats.json"),  # shared with seaice_xgboost
        "drift_columns": ["concentration"],
    },
    "trajectory_lstm": {
        "metric_name": "rmse_lat",
...
def check_and_retrain(model_name: str, new_data_df=None, train_fn=None, force: bool = False) -> dict:
    ...
    if new_data_df is not None:
        try:
            ...
            drift_warnings = detect_drift(new_data_df, ref_stats, config["drift_columns"])
        except FileNotFoundError:
            logger.info(f"No reference statistics yet for {model_name} — skipping drift check this run")

    decision = should_retrain(model_name, config["metric_name"], drift_warnings=drift_warnings)

    if not force and not decision["retrain"]:
        return {"action": "skipped", "reasons": decision["reasons"] or ["no trigger conditions met"]}

    if train_fn is None:
        return {"action": "retrain_needed_but_no_train_fn_provided", "reasons": decision["reasons"]}

    logger.info(f"Retraining {model_name}. Reasons: {decision['reasons'] or ['forced']}")
    train_fn()

    history = get_experiment_history(model_name)
    if not history:
        return {"action": "retrain_failed", "reason": "training ran but no record was logged"}
        
    latest_run = history[-1]
    new_metric = latest_run.get("metrics", {}).get(config["metric_name"])
    if new_metric is None:
        return {"action": "retrain_failed", "reason": "training ran but no metric was logged"}
        
    candidate_model_path = latest_run.get("artifact_uri")
    if not candidate_model_path:
        return {"action": "retrain_failed", "reason": "training ran but no artifact_uri was logged"}
...
def test_force_retraining(mock_should_retrain, mock_get_latest_metric, mock_promote_if_better):
    # Even if should_retrain says no, --force overrides it
    mock_should_retrain.return_value = {"retrain": False, "reasons": []}
    mock_get_latest_metric.return_value = 1.2
    mock_promote_if_better.return_value = {"promoted": True, "reason": "Better than production"}
    ...
    mock_get_latest_metric.assert_called_once_with("seaice_xgboost", MODEL_CONFIGS["seaice_xgboost"]["metric_name"])
    mock_promote_if_better.assert_called_once()

def test_retrain_and_promote(mock_should_retrain, mock_get_latest_metric, mock_promote_if_better):
...
def test_retrain_but_skip_promotion(mock_should_retrain, mock_get_latest_metric, mock_promote_if_better):
    mock_should_retrain.return_value = {"retrain": True, "reasons": ["degradation detected"]}
    mock_get_latest_metric.return_value = 2.5
    mock_promote_if_better.return_value = {"promoted": False, "reason": "Worse than production"}
...
def test_retrain_failed_no_metric(mock_should_retrain, mock_get_latest_metric):
    ...
    result = check_and_retrain("trajectory_xgboost", train_fn=mock_train_fn, force=False)
...
if __name__ == "__main__":
    ...
    if args.model == "seaice_xgboost":
        ...
        result = check_and_retrain("seaice_xgboost", train_fn=lambda: train_model(), force=args.force)
    elif args.model == "trajectory_xgboost":
        from ml.training.trajectory_train import train_model
        result = check_and_retrain("trajectory_xgboost", train_fn=lambda: train_model(), force=args.force)
    elif args.model == "seaice_convlstm":
        from ml.training.seaice_train_convlstm import train_convlstm
        result = check_and_retrain("seaice_convlstm", train_fn=lambda: train_convlstm(), force=args.force)
    elif args.model == "trajectory_lstm":
        from ml.training.trajectory_train_lstm import train_lstm
        result = check_and_retrain("trajectory_lstm", train_fn=lambda: train_lstm(), force=args.force)
