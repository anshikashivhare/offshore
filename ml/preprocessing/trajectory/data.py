...
def generate_synthetic_tracks(
    ...
    for the whole track — matches real ERA5 data and is what gives a
    sequence model something temporal to learn that XGBoost can't see.
    """
    ...
    for iceberg_id in range(num_icebergs):
        ...
        for t in range(num_steps):
            if time_varying_env:
                current_u += rng.normal(0, 0.001)
                current_v += rng.normal(0, 0.0005)
                wind_u += rng.normal(0, 0.0005)
                wind_v += rng.normal(0, 0.00025)
